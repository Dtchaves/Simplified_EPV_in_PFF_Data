...
PE_DIR = ROOT / "src/Pass/Pass_epv_success"
REQUIRED_COLUMNS = [
    "game_id",
    "game_event_id",
    "possession_event_id",
...
def run_filter_contract_checks() -> Dict[str, int]:
    ...
    for source in sources:
        ...
        source_df = source_df[source_df["pass_outcome_type"] == "C"].copy()
        if source_df.empty:
            continue
        ...
        assert labeled_df["pass_outcome_type"].eq("C").all(), (
            f"Found non-success outcomes after PE-success filter in {source.get('source_name')}."
        )
    ...
    assert rows_after_filters > 0, "No rows left after PE-success + reward filtering."
...
def run_pe_success_checkpoint() -> None:
    ...
    assert len(dataset) > 0, "PE-success dataset is empty."
    ...
    model = pe_soccermap.SoccerMapPassEPVSuccess(in_channels=16).eval()
    ...
    heatmap_dir = ROOT / "results/heatmaps/pe_success_checkpoint"
    metrics_dir = ROOT / "results/metrics/pe_success_checkpoint"
    ...
    with torch.no_grad():
        for i in range(sample_count):
            ...
            axes[2].set_title("PE-success output")
            for ax in axes:
                ax.set_xticks([])
                ax.set_yticks([])
            fig.suptitle(f"sample={i} label={true_value:.0f} pred={pred_value:.4f}")
            fig.tight_layout()
            fig.savefig(heatmap_dir / f"pe_success_example_{i}.png", dpi=200)
            plt.close(fig)
    ...
    pd.DataFrame(samples).to_csv(metrics_dir / "pe_success_examples.csv", index=False)

    summary = {
        "status": "PE_SUCCESS_CHECKPOINT_AUDIT_OK",
        ...
        "artifacts": {
            "examples_csv": str(metrics_dir / "pe_success_examples.csv"),
            "heatmap_dir": str(heatmap_dir),
        },
    }

    with (metrics_dir / "pe_success_summary.json").open("w", encoding="utf-8") as output_file:
        json.dump(summary, output_file, indent=2)

    print("PE_SUCCESS_CHECKPOINT_AUDIT_OK")
